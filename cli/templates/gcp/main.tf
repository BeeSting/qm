locals {
  ...
  runtime_account_id = trim(substr("${var.org_id}-qm-runtime", 0, 30), "-")
  runtime_network_name = length("${var.org_id}-qm-runtime") <= 63 ? "${var.org_id}-qm-runtime" : "${substr(var.org_id, 0, 51)}-${substr(sha256(var.org_id), 0, 8)}"
  bucket_name        = trim(substr("${var.project_id}-${var.org_id}-qm-data", 0, 63), "-")
}
...
resource "google_sql_database" "qm" {
  name     = "qm"
  instance = google_sql_database_instance.postgres.name
}

resource "google_sql_user" "qm" {
  name     = "qm"
  instance = google_sql_database_instance.postgres.name
...
resource "google_secret_manager_secret_version" "database_url" {
  ...
  secret_data = "postgresql://qm:${urlencode(random_password.database.result)}@localhost/qm?host=/cloudsql/${google_sql_database_instance.postgres.connection_name}"
}

resource "google_service_account" "runtime" {
  account_id   = local.runtime_account_id
  display_name = "qm Cloud Run runtime"
}
