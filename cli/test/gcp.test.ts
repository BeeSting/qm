...
test("Cloud Run deploy arguments bind immutable images, env, secrets, SQL, and core scaling", () => {
  const args = gcpDeployArgs({
    projectId: "acme-project",
    region: "us-central1",
    serviceName: "acme-qm-core",
    image: `us-central1-docker.pkg.dev/acme-project/acme-qm/core@sha256:${"a".repeat(64)}`,
    serviceAccount: "runtime@acme-project.iam.gserviceaccount.com",
    env: { ORG_ID: "acme", VALUE: "a,b" },
    secrets: { DATABASE_URL: "acme-qm-DATABASE_URL:latest" },
    core: true,
    public: true,
    network: "acme-qm-runtime",
    subnet: "acme-qm-runtime",
    cloudSqlConnectionName: "acme-project:us-central1:acme-qm-postgres",
  });
  assert.deepEqual(args.slice(0, 4), ["run", "deploy", "acme-qm-core", "--image"]);
  ...
  assert.ok(args.includes("--set-secrets=^~^DATABASE_URL=acme-qm-DATABASE_URL:latest"));
...
test("GCP resource names derive deterministically from config", () => {
  assert.equal(gcpServiceName(config, "core"), "acme-qm-core");
  assert.equal(gcpSecretName(config.gcp!, "CORE_SIGNING_SECRET"), "acme-qm-CORE_SIGNING_SECRET");
...
test("the GCP deployment manifest records immutable images and deployed revisions", () => {
  const manifest: GcpManifest = {
    ...
    revisions: { core: "acme-qm-core-00001-abc" },
  };
  assert.match(manifest.images.core!, /@sha256:[a-f0-9]{64}$/);
  assert.equal(manifest.revisions.core, "acme-qm-core-00001-abc");
});
...
test("Cloud Run binds required stored secrets and skips absent optional secrets", () => {
  ...
  assert.equal(secrets.CORE_SIGNING_SECRET, "acme-qm-CORE_SIGNING_SECRET:latest");
  assert.equal(secrets.ANTHROPIC_API_KEY, undefined);
  assert.throws(
    () => gcpServiceSecrets(config, "core", new Set()),
    /required Secret Manager secret acme-qm-[A-Z_]+ is missing/,
  );
});
