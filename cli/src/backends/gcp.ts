...
import { securityScreenEnv, type GcpConfig, type QmConfig } from "../config.ts";
...
function requireGcp(config: QmConfig): GcpConfig {
  if (!config.gcp) throw new CliError('target "gcp" requires a gcp block');
  return config.gcp;
}

function gcloud(config: QmConfig, args: string[]): string {
...
export function gcpServiceName(config: QmConfig, service: string): string {
  const full = `${config.orgId}-qm-${service}`;
...
function serviceUrl(config: QmConfig, service: ServiceName): string {
  const gcp = requireGcp(config);
...
function availableSecrets(config: QmConfig): Set<string> {
...
function writeManifest(config: QmConfig, outputs: GcpOutputs, manifest: GcpManifest): void {
  const dir = mkdtempSync(join(tmpdir(), "qm-gcp-manifest-"));
...
function storedSecrets(config: QmConfig): Map<string, string> {
...
async function secretsPush(ctx: DeployContext, envFile?: string): Promise<void> {
  ...
  for (const secret of computedSecrets(ctx.config).filter((item) => item.managedBy === "operator")) {
    ...
    const dir = mkdtempSync(join(tmpdir(), "qm-gcp-secret-"));
