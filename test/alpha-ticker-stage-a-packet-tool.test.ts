...
for (const deploymentRoot of deploymentRoots) {
  const toolRoot = `${deploymentRoot}/sandbox/tools/alpha-packet`;
  ...
  test(`${deploymentRoot} returns explicitly synthetic advisory envelopes`, () => {
  ...
  test(`${deploymentRoot} rejects real or unknown scopes`, () => {
  ...
  test(`${deploymentRoot} offers no refresh, SQL, write, fetch, or argument smuggling`, () => {
  ...
  test(`${deploymentRoot} tool descriptor is egress-free and the declared binary is executable`, () => {
